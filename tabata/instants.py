    ...
    * `_clf`:       le classifieur finalement appris (un arbre de décision.)
    ...
    def __init__(self, storename, phase=None, pos=0, name=""):
        ...
        self.feature_params = dict(
            range_width=None, range_sigma=range(5, 26, 10), max_order=2
        )
            ...
            for d in range(max_order):
                for i, colname in enumerate(colnames):
                    ...
                    for df in data:
                        ...
                        a = df[colname]
                        b = signal.savgol_filter(
                            a, window_length=w, polyorder=deg_poly, deriv=d + 1
                        )
                        c = signal.savgol_filter(
                            b, window_length=w, polyorder=deg_poly, deriv=0
                        )
                        r = r + [np.std(b - c)]
                    R[d, i] = max(r)
            epsilon[w] = R
    ...
    def fit(self, progress_bar=None, message_label=None):
        ...
        def find_best_parameters(p, split, cols=None):
            ...
            for df in dsi:
                ...
                y = pd.DataFrame(
                    1 - 2 * (pos <= ind), index=x.index, columns=["TARGET"]
                )
                i0 = i0 + len(x)
                Y.append(y)
                X.append(x)
        ...
        if dsi:
            ...
            if colnames != self.variables or list(dsi.computed.values()) != list(
        ...
        if dsi is None:
            ...
            dsi = self.make_indicators(
                progress_bar=progress_bar, message_label=message_label
            )
        ...
        for k in range(rn):
            clf = find_best_parameters(
                self.learn_params["samples_percent"],
        ...
        print(
            "First keeping {} indicators over {}".format(
                len(keepcols), len(dsi.idcodes)
            )
        )
        p1 = self.learn_params["samples_percent"] * self.learn_params["retry_number"]
        clf = find_best_parameters(
            min(0.5, p1), self.learn_params["min_samples_split"], keepcols
        )
        fi = clf.feature_importances_
        if progress_bar:
            progress_bar.value += 1
        ...
        if progress_bar:
            progress_bar.value += 1
        print(
            "Then  keeping {} indicators over {}".format(
                len(keepcols), len(dsi.idcodes)
            )
        )
    ...
    def describe(self):
        ...
        codes = pd.DataFrame(
            data=self.idcodes, columns=["Name", "Filter", "Order", "Sigma", "Std"]
        )
    ...
    def belief(self, arg=None):
        ...
        p = signal.savgol_filter(
            ip, window_length=2 * filter_width + 1, polyorder=2, deriv=1
        )
    ...
    def make_figure(self, f, phase=None, pos=None, name=None):
        ...
        def update_plot(colname, sigpos, width):
            ...
            if self.sigpos in self.selected:
                ...
                f.add_shape(
                    ...
                    x1=x0,
                    y1=y1,
                    line={"color": "rgb(171, 50, 96)", "width": 2, "dash": "dashdot"},
                    row=1,
                    col=1,
                )
            if self.sigpos in self.computed:
                ...
                f.add_shape(
                    ...
                    x1=x0,
                    y1=y1,
                    line={"color": "rgb(96, 50, 171)", "width": 2, "dash": "dot"},
                    row=1,
                    col=1,
                )
        ...
        wf = widgets.IntSlider(
            value=self.predict_params["filter_width"],
            min=10,
            max=500,
            step=10,
            orientation="horizontal",
    ...
    def param(self):
        ...
        def update_parameters(
            width, sigma, max_order, retry, percentile, sample, split
        ):
        ...
        wfmo = widgets.IntText(
            description="Max Order", value=self.feature_params["max_order"]
        )
        ...
        wtrn = widgets.IntText(
            description="Retry", value=self.learn_params["retry_number"]
        )
        wtrp = widgets.IntText(
            description="Percentile", value=self.learn_params["retry_percentile"]
        )
        wtsp = widgets.FloatText(
            description="Sample", value=self.learn_params["samples_percent"]
        )
        wtss = widgets.FloatText(
            description="Split", value=self.learn_params["min_samples_split"]
        )
    ...
    def plot(self, phase=None, pos=None, name=None):
        ...
        learn = widgets.VBox(
            [
                ...
                widgets.HBox(
                    [
                        e["variable_selection"],
                        widgets.VBox([e["message_label"], e["learn_button"]]),
                    ]
                ),
            ]
        )
