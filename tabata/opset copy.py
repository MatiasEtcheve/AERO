...
from .plots import get_colname, nameunit
...
class Opset:
    ...
    @classmethod
    def from_generator(cls, generator, storename):

        pd.HDFStore(storename, mode="w").close()

        for df in generator:
            record_name = "/" + df.index.name
            df.to_hdf(storename, record_name)
    ...
    def __repr__(self):
        ...
        phase surlignée   : phase   = {}".format(
            self.storename, len(self.records), self.sigpos, self.colname, self.phase
        )
        ...
        if pos <= -len(self.records) or pos >= len(self.records):
            raise OpsetError(
                ...
                "La position doit être comprise entre {} \
                            et {}".format(
                    -len(self.records), len(self.records) - 1
                ),
            )
    ...
    def make_figure(self, f, phase=None, pos=None, name=None):
        ...
        * `update_function`:    la fonction de mise à jour de
        ...
        f.add_trace(
            go.Scatter(x=self.df.index, y=self.df[self.colname], name="value"),
            row=1,
            col=1,
        )
        if self.phase:
            ...
            #     col=1,
            # )
            self.unique_occurence = set()
            self.max_occurence = 0
            for df in self:
                self.unique_occurence = set(self.unique_occurence).union(
                    set(df[self.phase].unique())
                )
                for unique in df[self.phase].unique():
                    occurence_as_array = (df[self.phase] == unique).astype(int).tolist()
                    diff = np.diff([occurence_as_array[0]] + occurence_as_array)
                    count = np.sum(diff == 1)
                    self.max_occurence = max(self.max_occurence, count)

            colors = px.colors.qualitative.Set1
            beginning = 0
            ending = 0
            global_ind = {occurence: 0 for occurence in self.unique_occurence}
            indices_color = {
                occurence: colors[list(global_ind).index(occurence) % len(colors)]
                for occurence in global_ind.keys()
            }
            current_ind = None
            while ending < len(self.df[self.phase]):
                current_ind = self.df[self.phase][beginning]

                while (
                    ending < len(self.df[self.phase])
                    and self.df[self.phase][ending] == current_ind
                ):
                    ending += 1

                global_ind[current_ind] = global_ind.get(current_ind, 0) + 1
                f.add_trace(
                    go.Scatter(
                        x=self.df.index[beginning:ending],
                        y=self.df[self.colname].loc[beginning:ending],
                        name=f"phase={current_ind}:{global_ind[current_ind]}",
                        line={"color": indices_color[current_ind]},
                    ),
                    row=1,
                    col=1,
                )
                beginning = ending
            for ind, occurence in global_ind.items():
                for i in range(occurence, self.max_occurence + 1):
                    f.add_trace(
                        go.Scatter(
                            x=[],
                            y=[],
                            name=f"phase={ind}:{i}",
                            line={"color": indices_color[ind]},
                        ),
                        row=1,
                        col=1,
                    )
        ...
        def update_plot(colname, sigpos):
            ...
            f.update_traces(
                selector=dict(name="value"), x=self.df.index, y=self.df[self.colname]
            )
            ...
            if self.phase is not None:
                ...
                while ending < len(self.df[self.phase]):
                    ...
                    f.update_xaxes(
                        ...
                        range=[self.df.index[0], self.df.index[-1]],
                    )
                    f.update_yaxes(
                        selector=dict(
                            name=f"phase={current_ind}:{global_ind[current_ind]}"
                        ...
                        range=[
                            self.df[self.colname].min(),
                            self.df[self.colname].max(),
                        ],
                    )
                    beginning = ending
                # for ind, occurence in global_ind.items():
            ...
            f.update_layout(
                title=self.df.index.name, yaxis_title=name + "  [ " + unit + " ]"
            )
        ...
        wd = widgets.Dropdown(
            options=self.df.columns, value=self.colname, description="Variable :"
        )
    ...
    def plotc(self, phase=None, pos=None, name=None):
        ...
        out = widgets.interactive_output(
            update_plot_c,
            dict(colname=e["variable_dropdown"], sigpos=e["signal_slider"]),
        )
        boxes = widgets.VBox(
            [
                widgets.HBox(
