...
def selplot(df, variable=None, sep="["):
    ...
    def selected_plot(col):
        ...
        layout = go.Layout(
            ...
            xaxis={"title": df.index.name, "titlefont": {"color": "blue"}},
            yaxis={"title": unit, "titlefont": {"color": "blue"}},
        )
...
def byunitplot(df, yunit=None, title="", sep="["):
    ...
    def unit_plot(unit, variable):
        ...
        if variable == "All" or variable is None:
            cols = dnu[unit]
            data = [
                go.Scatter(x=df.index, y=df[col], name=nameunit(col)[0]) for col in cols
            ]
        else:
            data = [go.Scatter(x=df.index, y=df[variable], name=nameunit(variable)[0])]
        layout = go.Layout(
            title=title,
            titlefont={"color": "blue"},
            xaxis={"title": df.index.name, "titlefont": {"color": "blue"}},
    ...
    wv = widgets.Dropdown(
        options=["All"] + dnu[units[0]], value="All", description="Variables :"
    )
...
def groupplot(df, title="", standardize=False):
    ...
    if standardize:
        data = [
            go.Scatter(
                x=df.index, y=(df[col] - df[col].mean()) / df[col].std(), name=col
            )
...
def doubleplot(df1, df2=None, p=0.5, space=0.05, title=None):
    ...
    *Exemples*
    ...
    fig.update_xaxes(
        title_text=df1.index.name, titlefont={"color": "blue"}, row=2, col=1
    )

    [
        fig.add_trace(go.Scatter(x=df1.index, y=df1[col], name=col), row=1, col=1)
    ...
    [
        fig.add_trace(go.Scatter(x=df2.index, y=df2[col], name=col), row=2, col=1)
    ...
    if title:
        fig.update_layout(titlefont={"color": "blue"}, title_text=title)
...
def pcacircle(df, pca=None, sample=0):
    ...
    cnames = [
        "PC{} ({:.1f}%)".format(c, pca.explained_variance_ratio_[c - 1] * 100)
    ...
    def update_circle(cname1, cname2):
        ...
        total_variance2 = (
            pca.explained_variance_ratio_[comp1] + pca.explained_variance_ratio_[comp2]
        )
        layout = go.Layout(
            title="Projection dans le plan PC{} x PC{} ({:.1f}%)".format(
                comp1 + 1, comp2 + 1, total_variance2 * 100
            ),
